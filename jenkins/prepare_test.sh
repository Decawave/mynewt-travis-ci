...
case $TEST in
  "TEST_ALL")
    ...
    ;;
  "BUILD_TARGETS")
      target_list=$(find targets -iname pkg.yml -exec grep -H "pkg\.type:.*target" {} \;| cut -d: -f1 | sed s#/pkg.yml##g|sed "s/^targets.//"|sort|grep -v -e "^unittest\$")
...
echo ${sets[$((TARGET_SET - 1))]} > ${WORKSPACE}/targets.txt
